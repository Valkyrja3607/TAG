...
class CaSED(VocabularyFreeCLIP):
    ...
    def batch_step(
        self, images_z: torch.Tensor, vocabularies: list[list], return_only_vocabulary: bool = False
        ...
        return self.classifier_cosine(images_z, words_z, mask=mask), words, vocabularies
    
    def classifier_cosine(self, images_z: torch.Tensor, words_z: torch.Tensor, mask: torch.Tensor = None) -> torch.Tensor:
        # torch.Size([60, 768]), torch.Size([1, 244, 768]), torch.Size([60, 244])
        return torch.einsum("nf,mf->nm", images_z, words_z[0]) * mask
