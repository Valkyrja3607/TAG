...
from .tag_wrapper import TAGWrapper
...
def create_tag_model(
        ...
        database_name: str = "ViT-L-14_PMD_TOP5",
        n_clusters: int = 15,
    ...
    tag_model = TAGWrapper(model=open_clip_model, tokenizer=tokenizer, depth=gem_depth,
                           ss_attn_iter=ss_attn_iter, ss_attn_temp=ss_attn_temp, classes=classes,
                           model_name=model_name, pretrained=pretrained, database_name=database_name,
                           n_clusters=n_clusters, vocab_transform=vocab_transform, num_samples=num_samples,)
    logging.info(f'Loaded TAG-{model_name} from pretrained weights {pretrained}!')
    return tag_model
...
def create_model_and_transforms(
    ...
    gem_model = create_tag_model(model_name, pretrained, gem_depth, precision, device, jit, force_quick_gelu, force_custom_text,
                                 force_patch_dropout, force_image_size, force_preprocess_cfg, pretrained_image,
                                 pretrained_hf, cache_dir, output_dict, require_pretrained, **model_kwargs)
...
def visualize(image, text, logits, alpha=0.6, save_path=None):
    ...
    for viz, cls_name in zip(vizs, text):
        ...
        if save_path is not None:
            plt.savefig(f'{save_path}/heatmap_{cls_name}.png')
